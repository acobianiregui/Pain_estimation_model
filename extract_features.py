"""
Biosignal Feature Extraction for Pain Regression
================================================
Extracts features from sliding/non-overlapping windows across subjects.
...
Columns expected:
    Seconds,Ecg,Eda_E4,Eda_RB,Bvp,Emg,Resp,COVAS,Tmp
...
def safe_entropy(x, m=2, r_factor=0.2):
    """
    Approximate sample entropy (simple scalar implementation).
    Returns 0.0 when not enough variation exists.
    """
...
def basic_stats(arr, prefix):
    ...
    return {
        ...
        f"{prefix}_std": np.std(arr, ddof=1) if len(arr) > 1 else 0.0,
        f"{prefix}_min": np.min(arr),
        f"{prefix}_max": np.max(arr),
        f"{prefix}_skew": float(stats.skew(arr)) if len(arr) > 2 else 0.0,
        f"{prefix}_kurt": float(stats.kurtosis(arr)) if len(arr) > 3 else 0.0,
    }
...
def ecg_features(ecg_window, fs=FS):
    ...
    try:
        ...
        if len(peaks) >= 2:
            ...
            feats["ecg_rr_std_ms"] = np.std(rr_intervals, ddof=1) if len(rr_intervals) > 1 else 0.0
            feats["ecg_rr_range_ms"] = np.ptp(rr_intervals)

            if len(rr_intervals) >= 2:
                diff_rr = np.diff(rr_intervals)
                feats["ecg_rmssd"] = np.sqrt(np.mean(diff_rr ** 2))
                nn50 = np.sum(np.abs(diff_rr) > 50)
                feats["ecg_pnn50"] = nn50 / len(diff_rr) * 100 if len(diff_rr) > 0 else 0.0
...
def eda_features(eda_window, prefix, fs=FS):
    ...
    for k in [
        ...
        f"{prefix}_scl_std",
        f"{prefix}_scl_slope",
        f"{prefix}_scr_n_peaks",
        f"{prefix}_scr_peak_rate",
        f"{prefix}_scr_mean_amp",
        f"{prefix}_scr_max_amp",
        f"{prefix}_scr_std",
        f"{prefix}_scr_auc",
        f"{prefix}_scr_energy",
    ]:
        feats[k] = np.nan
    ...
    try:
        ...
        feats[f"{prefix}_scl_std"] = np.std(scl, ddof=1) if len(scl) > 1 else 0.0
        feats[f"{prefix}_scl_slope"] = np.polyfit(np.arange(len(scl)), scl, 1)[0]

        feats[f"{prefix}_scr_std"] = np.std(scr_rect, ddof=1) if len(scr_rect) > 1 else 0.0
        feats[f"{prefix}_scr_auc"] = np.trapz(scr_rect, dx=1 / fs)
        feats[f"{prefix}_scr_energy"] = np.sum(scr_rect ** 2) / fs

        peak_height = 0.01 * np.ptp(eda_window) if np.ptp(eda_window) > 0 else 0.001
        ...
        feats[f"{prefix}_scr_peak_rate"] = len(peaks) / (len(eda_window) / fs)
...
def bvp_features(bvp_window, fs=FS):
    ...
    try:
        feats["bvp_slope"] = np.polyfit(np.arange(len(bvp_window)), bvp_window, 1)[0]
        ...
        if len(peaks) >= 2:
            ...
            feats["bvp_ibi_std_ms"] = np.std(ibi, ddof=1) if len(ibi) > 1 else 0.0
            feats["bvp_ibi_cv"] = (
                feats["bvp_ibi_std_ms"] / feats["bvp_ibi_mean_ms"]
                if feats["bvp_ibi_mean_ms"] > 0 else np.nan
            )
            feats["bvp_pulse_amp_mean"] = np.mean(bvp_window[peaks])
            feats["bvp_pulse_amp_std"] = np.std(bvp_window[peaks], ddof=1) if len(peaks) > 1 else 0.0
...
def emg_features(emg_window, fs=FS):
    ...
    try:
        ...
        feats["emg_wl"] = np.sum(np.abs(np.diff(emg_window)))
        ...
        if pos.any() and np.sum(psd[pos]) > 0:
            cum_pow = np.cumsum(psd[pos]) / np.sum(psd[pos])
            feats["emg_median_freq"] = freqs[pos][np.searchsorted(cum_pow, 0.5)]
            feats["emg_mean_freq"] = np.sum(freqs[pos] * psd[pos]) / np.sum(psd[pos])
            feats["emg_total_power"] = np.trapz(psd[pos], freqs[pos])

            low_mask = (freqs >= 20) & (freqs < 80)
            high_mask = (freqs >= 80) & (freqs <= 150)
            ...
            feats["emg_low_band_power"] = low_power
            feats["emg_high_band_power"] = high_power
            feats["emg_hl_ratio"] = high_power / low_power if low_power > 0 else np.nan
...
def resp_features(resp_window, fs=FS):
    ...
    for k in [
        "resp_slope",
        "resp_rate_bpm",
        "resp_interval_mean_s",
        "resp_interval_range_s",
        "resp_irregularity_idx",
        "resp_amp_mean",
        "resp_amp_std",
        "resp_dominant_freq",
    ]:
    ...
    try:
        feats["resp_slope"] = np.polyfit(np.arange(len(resp_window)), resp_window, 1)[0]
...
def tmp_features(tmp_window):
    ...
    try:
        feats["tmp_slope"] = np.polyfit(np.arange(len(tmp_window)), tmp_window, 1)[0]
        feats["tmp_range"] = np.ptp(tmp_window)
        feats["tmp_diff_end_start"] = tmp_window[-1] - tmp_window[0]
    except Exception:
        pass
...
def process_subject(filepath, subject_id, window_samples, step_samples):
    ...
    required = ["Seconds", "Ecg", "Eda_E4", "Eda_RB", "Bvp", "Emg", "Resp", "Tmp", "COVAS"]
    missing = [c for c in required if c not in df.columns]
    if missing:
        print(f"    WARNING: missing columns {missing} in subject {subject_id}. Skipping.")
        return None

    sig_cols = ["Ecg", "Eda_E4", "Eda_RB", "Bvp", "Emg", "Resp", "Tmp", "COVAS"]
...
def get_features(
    ...
    ----------
    input_folder : str
        Folder containing per-subject CSV files.
    output_folder : str
        Folder where output feature CSVs will be saved.
    window_length : float
        Window length in seconds.
    overlap : bool
        Whether windows overlap.
    overlap_percentage : float
        Overlap percentage in [0, 50].
    """
